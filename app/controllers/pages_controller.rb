class PagesController < ApplicationController
  ...
  def home
    @memos = Memo.all
    @memos_today = @memos.where("date = ? AND categorie = ?", Date.today, "Ne pas oublier")
    week = Date.today.strftime("%W").to_i + 1
    @memos_this_week_except_today = @memos.select { |memo| memo.date.strftime("%W").to_i + 1 == week }.select { |memo| memo.categorie == "Ne pas oublier" }.reject { |memo| memo.date == Date.today }
    arecolter_ajd
    arecolter_j1
    arecolter_j2

    week = Date.today.strftime("%W").to_i + 1
    @todo_this_week = @memos.select { |memo| memo.date.strftime("%W").to_i + 1 == week }.select { |memo| memo.categorie == "À faire" }
